...
void Bonus_void_CheckDirection(u8 place){
  ...
  u8 input;
    ...
    static const struct Landmark landmarks[] = {
        ...
        GPIO_u8ConfigPin(Port1_For_Locations,place,GPIO_INPUT,0,GPIO_PD);
        if(GPIO_u8GetPinValue(Port1_For_Locations,place,&input)&&input==GPIO_OUT_HIGH)
				{
        UART1_voidReceiveString(gps_string);
        GPS_u8ParsGpsString(gps_string,&GPS_parsed);
        }
        ...
        GPIO_u8ConfigPin(Port2_For_Locations,place-8,GPIO_INPUT,0,GPIO_PD);
        if(GPIO_u8GetPinValue(Port2_For_Locations,place-8,&input)&&input==GPIO_OUT_HIGH)
				{
        UART1_voidReceiveString(gps_string);
        GPS_u8ParsGpsString(gps_string,&GPS_parsed);
        }
